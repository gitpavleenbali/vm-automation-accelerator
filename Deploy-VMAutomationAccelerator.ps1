...
function Invoke-TerraformDeploy {
    ...
    try {
        ...
        if ($env:ARM_CLIENT_ID -and $env:ARM_CLIENT_SECRET -and $env:ARM_TENANT_ID) {
            ...
            $autoTfvarsContent = @"
# Auto-generated by pipeline - DO NOT COMMIT
# Service Principal authentication credentials
arm_client_id     = "$env:ARM_CLIENT_ID"
arm_client_secret = "$env:ARM_CLIENT_SECRET"
arm_tenant_id     = "$env:ARM_TENANT_ID"
"@
            Set-Content -Path $autoTfvarsPath -Value $autoTfvarsContent -Force
            Write-Success "✓ Created pipeline-auth.auto.tfvars with Service Principal credentials"
            
            # Also build command-line args as backup
            $script:TerraformAuthVars = @(
        ...
        # Clean up pipeline auth file if it exists
        $autoTfvarsPath = Join-Path $WorkingDirectory "pipeline-auth.auto.tfvars"
        if (Test-Path $autoTfvarsPath) {
            Remove-Item $autoTfvarsPath -Force -ErrorAction SilentlyContinue
            Write-Info "Cleaned up pipeline-auth.auto.tfvars"
        }
